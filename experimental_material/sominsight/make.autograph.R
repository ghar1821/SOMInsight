#' Spectre's make.autograph - Creates plots using ggplot2. Some adaptation was made to the original Spectre's autograph function.
#'
                           ...
                           )
{
  ...
  message("AutoGraph - plotting started")
      ...
      ## COLOUR CONTROL
          ...
          p <- p + ggplot2::labs(x= x.axis.label,
                                 y = y.axis.label,
    ...
    if(scale == "lin"){
      p <- p + ggplot2::scale_y_continuous(limits = c(0, max_y_value_p40), breaks = scales::pretty_breaks(10),
                                           labels = scales::comma)
    }
